        ...
        for(int i = 0; i < 8; i++){
            for(int j = 0; j < 8; j++){
                ...
                rect.setClickAction(event -> {
                    if(selectedPiece >= 0){
                        int position = ChessBoard.positionToInt(row, column);
    ...
    private void updatePanel() {
        ...
        for(int i = 0; i < 8; i++){
            for(int j = 0; j < 8; j++){
                ...
                if(value != 0) {
                    ChessBoard.ChessPiece piece = ChessBoard.ChessPiece.getPiece(value);
                    new GraphicPiece(panel, -400 + j * 100, -400 + (7 - i) * 100, piece);
                }

                int pos = ChessBoard.positionToInt(i, j);
/*                if(showControls) {
                    if (board.isAttackedBy(pos, true)) {
                        attackedPositionsNodes.add(new RectangleNode(panel, -400 + j * 100, -400 + (7 - i) * 100, 100, 100, new Color(0, 255, 0, 115)));
                    }
                    if (board.isAttackedBy(pos, false)) {
                        attackedPositionsNodes.add(new RectangleNode(panel, -400 + j * 100, -400 + (7 - i) * 100, 100, 100, new Color(255, 0, 0, 115)));
                    }
                }*/
            }
        }
    ...
    public void setSelectedPiece(int row, int column) {
        ...
        if(selectedPiece != position) {
            ...
            ChessBoard.ChessPiece piece = ChessBoard.ChessPiece.getPiece(id);
            if (piece.isWhite() == board.isWhiteToPlay()) {
                ...
                for (AbstractChessMove allowed : allowedMoves) {
                    int[] allowedPos = ChessBoard.intToPosition(allowed.getFinalPosition());
